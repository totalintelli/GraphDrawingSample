...
namespace WinForm
{
    public partial class Form1 : Form
    {
        ...
        private void panel1_Paint(object sender, PaintEventArgs e)
        {
            ...
            // 실제 타입에 따른 그래프 그리는 구역
            if (isDrwaing && Width > 80 && Height > 80)
            {
                // 원을 그린다.
                DrawCircle(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 타원을 그린다.
                DrawEllipse(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 계란을 그린다.
                DrawOval(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 네 잎을 그린다.
                // 북쪽에 있는 잎을 그린다.
                DrawFourLeaf(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 곡선으로 이루어진 삼각형을 그린다.
                DrawCurvilinearTriangle(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 정사각형을 그린다.
                DrawSquare(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 직사각형을 그린다.
                DrawRectAngle(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 삼각형을 그린다.
                DrawTriangle(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 사다리꼴을 그린다.
                DrawTrapezoid(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 마름모를 그린다.
                DrawRhombus(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 오각형을 그린다.
                DrawPentagon(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 육각형을 그린다.
                DrawHexagon(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 팔각형을 그린다.
                DrawOctagon(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 평행사변형을 그린다.
                DrawParallelogram(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);

                // 초승달을 그린다.
                DrawCrescent(e.Graphics, DrawingWidth, DrawingHeight, FigureColor, RectMargin);
            }
        }

        private void DrawCircle(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 원을 그리는 사각형
            RectangleF CircleRect = new RectangleF(RectMargin.Left, RectMargin.Top, DrawingWidth, DrawingHeight);

            // 원을 그린다.
            gr.FillEllipse(FigureColor, CircleRect);
        }

        private void DrawEllipse(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 너비의 2배
        ...
        private void DrawOval(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 너비의 2배
            float DrawingWidthDouble = DrawingWidth * 2.0f;
            // 드로잉 너비의 3배
        ...
        private void DrawFourLeaf(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 너비의 3배
            float DrawingWidthTriple = DrawingWidth * 3.0f;
            // 드로잉 구역의 간격의 너비
        ...
        private void DrawCurvilinearTriangle(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 너비의 4배
        ...
        private float GetRadius(float DrawingWidth, float DrawingHeight)
        {
            // 정다각형을 둘러싸는 원의 지름
            float Radius = 0.0f;
        ...
        private void DrawSquare(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 구역의 간격의 높이
            float GapHeight = DrawingHeight * 0.1f;                                                                      // 0.1f는 드로잉 구역의 간격의 높이의 비율로 고정값.
            // // 정다각형을 둘러싸는 원의 지름을 구한다.
            float Radius = GetRadius(DrawingWidth, DrawingHeight);
            // 정사각형을 그리는 사각형
        ...
        private void DrawRectAngle(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 구역의 간격의 너비
            float GapWidth = DrawingWidth * 0.1f;                                                                                  // 0.1f는 드로잉 구역의 간격의 너비의 비율로 고정값.
            // 드로잉 구역의 간격의 높이
        ...
        private void DrawTriangle(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 구역의 간격의 높이
            float GapHeight = DrawingHeight * 0.1f;                                                                             // 0.1f는 드로잉 구역의 간격의 높이의 비율로 고정값.
            // 정다각형을 둘러싸는 원의 지름을 구한다.
        ...
        private void DrawTrapezoid(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 너비의 3배
            float DrawingWidthTriple = DrawingWidth * 3.0f;
            // 드로잉 너비의 4배
        ...
        private void DrawRhombus(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 너비의 4배
            float DrawingWidthQuad = DrawingWidth * 4.0f;
            // 드로잉 높이의 2배
        ...
        private void DrawPentagon(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 정다각형을 둘러싸는 원의 지름을 구한다.
            float Radius = GetRadius(DrawingWidth, DrawingHeight);
            // 드로잉 높이의 2배
        ...
        private void DrawHexagon(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 높이의 2배
            float DrawingHeightDouble = DrawingHeight * 2.0f;
            // 드로잉 구역의 간격의 높이
        ...
        private void DrawOctagon(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 너비의 2배
            float DrawingWidthDouble = DrawingWidth * 2.0f;
            // 드로잉 높이의 2배
        ...
        private void DrawParallelogram(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 너비의 3배
            float DrawingWidthTriple = DrawingWidth * 3.0f;
            // 드로잉 너비의 4배
        ...
        private void DrawCrescent(Graphics gr, float DrawingWidth, float DrawingHeight, SolidBrush FigureColor, Padding RectMargin)
        {
            // 드로잉 너비의 4배
            float DrawingWidthQuad = DrawingWidth * 4.0f;
            // 드로잉 구역의 간격의 너비
            ...
            float GapWidthQuad = GapWidth * 4.0f;
            // 드로잉 높이의 2배                                                                                                     
            float DrawingHeightDouble = DrawingHeight * 2.0f;                                                                  // 0.1f는 드로잉 구역의 간격의 높이의 비율로 고정값.
            // 드로잉 구역의 간격의 높이
            float GapHeight = DrawingHeight * 0.1f;
            // 드로잉 구역의 간격의 높이의 2배
            float GapHeightDouble = GapHeight * 2.0f;
